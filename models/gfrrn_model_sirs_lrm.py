...
class GFRRNModelBase(BaseModel):
    ...
    def set_input(self, data, mode='train'):
        ...
        mode = mode.lower()
        if mode == 'train':
            input, target_t, target_r, target_r_low, target_r_high, data_name = data['input'], data['target_t'], data['target_r'], data['target_r_low'], data['target_r_high'], data['fn']
        elif mode == 'eval':
            input, target_t, target_r, target_r_low, target_r_high, data_name = data['input'], data['target_t'], data['target_r'], data['target_r_low'], data['target_r_high'], data['fn']
        elif mode == 'test':
            input, data_name, target_r_low, target_r_high, target_r = data['input'], data['fn'], None, None, None
        else:
        ...
        if target_r_low is not None:
            target_r_low = target_r_low.to('cuda:0')
        if target_r_high is not None:
            target_r_high = target_r_high.to('cuda:0')

        self.input = input
        self.identity = identity
        self.target_t = target_t
        self.target_r = target_r
        self.target_r_low = target_r_low
        self.target_r_high = target_r_high
    ...
    def eval(self, data, savedir=None, suffix=None, pieapp=None):
        ...
        with torch.no_grad():
            ...
            output_r_low = tensor2im(self.output_r_low)
            output_r_high = tensor2im(self.output_r_high)
            ...
            if savedir is not None:
                if self.data_name is not None:
                    ...
                    Image.fromarray(output_r_low.astype(np.uint8)).save(
                        join(savedir, '{}_r_low.png'.format(self.opt.name)))
                    Image.fromarray(output_r_high.astype(np.uint8)).save(
                        join(savedir, '{}_r_high.png'.format(self.opt.name)))
...
class GFRRNModel(GFRRNModelBase):
    def name(self):
        return 'gfrrn_lrm'
    ...
    def get_loss(self, out_t, out_r_low):
        loss_t_pixel = self.loss_dic['t_pixel'](out_t, self.target_t)
        loss_r_low_pixel = self.loss_dic['r_low_pixel'](out_r_low, self.target_r_low)
        loss_r_high_pixel = self.loss_dic['r_high_pixel'](self.output_r_high, self.target_r_high) * self.opt.lamda_high_residual
        loss_t_vgg = self.loss_dic['vgg'](out_t, self.target_t) * self.opt.lambda_vgg
        loss_exclu = self.loss_dic['exclu'](self.output_t, self.output_r_low)
        loss_recons = self.loss_dic['recons'](
            self.output_t, self.output_r_low, self.output_r_high, self.input) * self.opt.lambda_rec
        return loss_t_pixel, loss_r_low_pixel, loss_r_high_pixel, loss_t_vgg, loss_exclu, loss_recons

    def backward_G(self):
        (self.loss_t_pixel, self.loss_r_low_pixel, self.loss_r_high_pixel, 
         self.loss_t_vgg, self.loss_exclu, self.loss_recons) = self.get_loss(self.output_t, self.output_r_low)
        self.loss_G = self.loss_t_pixel + self.loss_r_low_pixel+ self.loss_r_high_pixel + self.loss_t_vgg + self.loss_exclu + self.loss_recons
        self.loss_G.backward()

    def forward(self):
        # without edge
        input_i = self.input
        output_t, output_r_low, output_r_high = self.network(input_i)
        self.output_t = output_t
        self.output_r_low = output_r_low
        self.output_r_high = output_r_high

        return output_t, output_r_low, output_r_high
    ...
    def get_current_errors(self):
        ...
        if self.loss_r_low_pixel is not None:
            ret_errors['R_Low'] = self.loss_r_low_pixel.item()
        if self.loss_r_high_pixel is not None:
            ret_errors['R_High'] = self.loss_r_high_pixel.item()
    ...
    def get_current_visuals(self):
        ...
        ret_visuals['output_r_low'] = tensor2im(self.output_r_low).astype(np.uint8)
        ret_visuals['output_r_high'] = tensor2im(torch.clip((self.output_r_high + 1) / 2, 0., 1.)).astype(np.uint8)
